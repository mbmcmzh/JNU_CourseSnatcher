...
class RequestSniffer:
    ...
    def _init_browser(self):
        ...
        chrome_temp_dir = os.path.join(
            tempfile.gettempdir(), f"jnu_sniffer_chrome_{pid}_{timestamp}"
        )
        ...
        try:
            ...
            if os.path.exists(chrome_exe_path):
                ...
                print(
                    "   如果您希望使用内置版，请确保Chrome浏览器文件夹位于程序根目录下的'chrome'文件夹中（最终路径为 chrome/chrome.exe）。"
                )
            # --- 内置Chrome定位结束 ---
            ...
            print("=" * 60)
            raise Exception(
                "Chrome 浏览器未安装或无法启动。请安装 Chrome 浏览器后重试。"
            )
                    ...
                    if request.response and target_url in request.url:
                        ...
                        if request.body:
                            try:
                                ...
                                payload_str = json.dumps(
                                    payload_json, indent=2, ensure_ascii=False
                                )
                        ...
                        if request.response.body:
                            try:
                                ...
                                # 如果不是JSON，则以纯文本显示
                                response_body_str = request.response.body.decode(
                                    "utf-8", errors="ignore"
                                )
...
def course_search(
    headers, student_code, electiveBatchCode, queryContent, teachingClassType="QXKC"
):
    ...
    payload = {"querySetting": query_settings_json}
...
def course_addParam_generate(
    ...
    for course_info in course_info_list:
        ...
        payload = {"addParam": addParam_json}
...
def main():
    ...
    captured_data = sniffer.sniff_requests(
        visit_url=BASE_URL, target_url=xkxf_URL, timeout=Browser_wait_time
    )
    ...
    if captured_data:
        ...
        course_classid_list = []
        while True:
            queryContent_input = input(
                "请输入课程的班号(输入空值结束，注意不要输入课程号，输入的班号是教学班号):"
            )
        ...
        electiveBatchCode = (
            captured_data["request_payload"].split("xklcdm=")[1].split("&")[0]
        )
        ...
        for queryContent in course_classid_list:
            course_info = course_search(
                headers, student_code, electiveBatchCode, queryContent
            )
            course_info_list.append(course_info)

        # 生成添加课程的参数
        addParam_list = course_addParam_generate(
            course_info_list, student_code, electiveBatchCode
        )
