...
def main():
    ...
    course_snatch_url = "https://jwxk.jnu.edu.cn/xsxkapp/sys/xsxkapp/elective/volunteer.do" # 用于发送抢课request的url
    ...
    # 第一次请求（获取JESSIONID和_WEU）
    cookies_1 = cookies_str_to_dict(cookies_str)
    header_1 = {
        "User-Agent": UA
    }
    response_1 = requests.get(base_url, headers=header_1, cookies=cookies_1)


    # 第二次请求（获取新的_WEU和抢课token）
    cookie_2 = build_cookie_string(response_1.cookies, cookies_1, response_1.cookies.get("_WEU")) # 在cookie中加入JESSIONID和_WEU
    headers_2 = {
        "Cookie": cookie_2,
        "User-Agent": UA
    }
    response_2 = requests.get(register_url, headers=headers_2)
    token_jnu = json.loads(response_2.text)['data']['token']

    # 第三次请求（抢课）
    cookie_3 = build_cookie_string(response_1.cookies, cookies_1, response_2.cookies.get("_WEU")) # 更新_WEU
    headers_3 = {
        "Cookie": cookie_3,
        "Token": token_jnu,
        "User-Agent": UA
    ...
    while True:
        for course in course_list:
            try:
                response_3 = requests.post(course_snatch_url, headers=headers_3, data=course['class_data'])
                print(f"\n{course['display_name']}")
                print(f"响应结果: {response_3.text}")
